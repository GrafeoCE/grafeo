...
- **Memory introspection for RDF and CDC components**: `db.memory_usage()` now includes `rdf` and `cdc` blocks alongside the existing store/indexes/MVCC/caches/string-pool/buffer-manager totals. The RDF block reports triple count, triples-and-indexes bytes, cached term dictionary size, optional Ring index size, and named-graph count (recursing through nested graphs). The CDC block reports total bytes, entity count, and event count. Both blocks are feature-gated and serde-skipped when empty, so LPG-only builds see no change to existing JSON output. Closes the Phase 4 memory-introspection scope from 0.5.40.
- **CLI `:memory` meta-command**: prints the hierarchical memory breakdown in the REPL. Zero-valued and feature-disabled components are omitted so the output stays terse on small or embedded databases.
- **`CompactStoreTiered` wrapper** (architecture/compact-store-mmap): new two-state wrapper around `CompactStore` that can move between `InMemory` and `OnDisk` backed by mmap. Exposes `persist(path)` (snapshot to disk without switching state), `persist_to_mmap(path)` (serialize, mmap, swap to on-disk state, return bytes written), `open_mmap(path)` (open an existing on-disk store), and `reload_to_ram()` (drop mmap, keep heap-owning store). Atomic file writes via temp + rename; unsafe `Mmap::map` call is isolated to one helper with a scoped `#[allow(unsafe_code)]`. Gated on `compact-store + mmap`. Primitive only: BufferManager/SectionConsumer lifecycle wiring and full `compact()` integration are a follow-up.
